...
def get_stations():
    ...
    for data in results:
        ...
        station_dict["Station_Name"]= data.name
...
def get_start_date(start):
    ...
    for data in results:
        temp_dict = {}
        data["Date"] = float(data[0])
        data["Average Temperature"] = float(data[1])
        data["Max Temperature"] = float(data[2])
        data["Min Temperature"] = float(data[3])
        temp_data.append(temp_dict)
